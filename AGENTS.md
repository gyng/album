...
- The basemap is the reader's choice: `util/mapStyles.ts` holds the curated MapTiler styles, the shared public key and the preference (a small external store, because a `localStorage` write raises no event in its own tab). `MapStyleToggle` picks — rendered over the map (`showStylePicker`), not in the site chrome, because two selects side by side are wider than a phone's nav row and took a band of it to themselves — and `MapWorld` reads it through `useMapStyleName`. Every option is the same provider and key as the default, so no option adds a credential or an attribution obligation — the map renders whatever attribution the loaded style declares. `Map.tsx` and `StatsWorldMap` keep their own deliberate styles
