...
class ViT(VisionModel):
    ...
    def generate_image(self, tag: str, iterations: int):
        # using this model's loss function, generate a bitmap images using gradient ascent
        # to maximize the probability of the given tag
        # Ensure the tag is valid
        with open(Path("joytag") / "top_tags.txt", "r") as f:
            top_tags = [line.strip() for line in f.readlines() if line.strip()]

        tag_idx = top_tags.index(tag) if tag in top_tags else None
        if tag_idx is None:
            raise ValueError(f"Tag '{tag}' not found in model tags.")
        ...
        #     device=next(self.parameters()).device,
        # )
        optimizer = torch.optim.Adam([image], lr=0.15)
        ...
        #     optimizer, step_size=int(iterations * 0.2), gamma=0.1
        # )
        scheduler = optim.lr_scheduler.StepLR(
        optimizer, step_size=int(iterations * 0.1), gamma=0.5
        )

        for step in range(iterations):  # Number of iterations for gradient ascent
            optimizer.zero_grad()
            # output = self.forward({"image": image})
            output = self.forward({"image": image}, return_loss=False)

            # print(output)
            # loss = output["loss"]
            # loss = -output["loss"]

            loss = -output["tags"][0, tag_idx]  # We want to maximize this tag's probability
           
            # loss = self.calculate_loss(output, {"tags": torch.tensor([tag_idx])}, None)

            # loss = -output["tags"][
            #     0, tag_idx
            # ]  # We want to maximize this tag's probability
            loss.backward()

            # now update original image to maximize the tag probability
            

            optimizer.step()
            scheduler.step()

            # Clamp the image values to be in the valid range
            with torch.no_grad():
                image.clamp_(0, 1)

            if step % (iterations / 10) == 0:
                print(
                    f"Step {step}, Loss: {loss.item()}, learning rate: {scheduler.get_last_lr()}"
                )
                # print(f'output: {output["tags"][0, tag_idx].item()}')
                # save a preview
                img = image[0].detach().cpu().numpy().transpose(1, 2, 0)
                img = (img * 255).astype(np.uint8)
                pil_img = Image.fromarray(img)
                path = f"generated/{tag}_preview_{step}.bmp"
                pil_img.save(path)
        ...
        img = (img * 255).astype(np.uint8)
        pil_img = Image.fromarray(img)
        path = f"generated/{tag}.bmp"
        pil_img.save(path)
        image_paths.append(path)

        return f"generated/{tag}.bmp", image_paths
