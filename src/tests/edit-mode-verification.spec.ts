...
test.describe("Edit Mode Verification Tests", () => {
    ...
    for (const route of editRoutes) {
      try {
        ...
        if (status === 200) {
          ...
          console.log(
            `✓ Edit route ${route} properly blocked (status ${status})`,
          );
        }
      } catch (error) {
  ...
  test("all core navigation still works without edit mode", async ({
    page,
  }) => {
    // Start from homepage
    await page.goto("/");
  ...
  test("theme toggle works without edit mode interference", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const themeToggle = page
      .locator(
        'button[title*="theme" i], button:has-text("🌙"), button:has-text("☀️")',
      )
      .first();
