...
const outputPath = pathArgument(
  "--output",
  path.join(__dirname, "..", "public", "e2e-search.sqlite"),
);
const embeddingsOutputPath = pathArgument(
  "--embeddings-output",
...
if (existsSync(outputPath) && existsSync(embeddingsOutputPath) && !force) {
  console.log(`Using existing E2E search databases at ${outputPath} and ${embeddingsOutputPath}`);
  process.exit(0);
}
...
console.log(
  `Created deterministic E2E search databases at ${outputPath} and ${embeddingsOutputPath}`,
);
