...
const Probe = ({ whenUnobservable }: { whenUnobservable?: boolean } = {}) => {
  const ref = React.useRef<HTMLDivElement>(null);
  const near = useNearViewport(ref, undefined, whenUnobservable);
  return (
...
describe("useNearViewport", () => {
  ...
  // A caller whose "near" branch is a WebGL map wants the opposite: with no way
  // to tell what is on screen, mounting all of them is worse than none.
  it("lets an expensive caller opt out when it cannot observe", () => {
    delete (globalThis as { IntersectionObserver?: unknown }).IntersectionObserver;

    render(<Probe whenUnobservable={false} />);
